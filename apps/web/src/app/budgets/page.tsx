...
import { Plus, AlertTriangle, CheckCircle2, TrendingUp, Pencil, Trash2 } from "lucide-react";
...
export default function BudgetsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Budget cards */}
      {budgets.length === 0 ? (
        <Card className="py-16 flex flex-col items-center gap-2 text-slate-400 dark:text-slate-500">
          <p className="text-sm">Nenhum orçamento criado ainda.</p>
          <Button size="sm" onClick={() => setShowNew(true)}><Plus size={14} /> Criar primeiro orçamento</Button>
        </Card>
      ) : (
      ...
      <BudgetModal open={showNew} onClose={() => setShowNew(false)} />
      <BudgetModal open={!!editing} onClose={() => setEditing(null)} initial={editing ?? undefined} />
    </div>
  );
}
